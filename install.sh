...
# Initialize state file (must be valid shell assignments)
: > "${STATE_FILE}"
...
if [ "${EXISTS_GAME}" -eq 0 ]; then
  sqlite3 "${DB_REGIONAL}" "INSERT INTO MENU_ENTRIES
    ...
    VALUES
    (${NEW_ID}, '${TITLE_SQL}', 'Sony Interactive Entertainment', 1994, 4, 'CERO_A', '', '', ${NEW_ID});"
...
# BleemSync.db: let SQLite assign Ids (most compatible with Eris)
EXISTS_NODE="$(sqlite3 "${DB_BLEEM}" "SELECT COUNT(*) FROM GameManagerNodes WHERE Name='${TITLE_SQL}';")"
if [ "${EXISTS_NODE}" -eq 0 ]; then
  sqlite3 "${DB_BLEEM}" "INSERT INTO GameManagerNodes (Name, Type, ParentId, Position)
    VALUES ('${TITLE_SQL}', 3, 0, ${NEW_ID});"

  NEW_NODE_ID="$(sqlite3 "${DB_BLEEM}" "SELECT Id FROM GameManagerNodes WHERE Name='${TITLE_SQL}' ORDER BY Id DESC LIMIT 1;")"
  log "Inserted GameManagerNodes row (Id=${NEW_NODE_ID})"

  sqlite3 "${DB_BLEEM}" "INSERT INTO GameManagerFiles (Name, Path, NodeId)
    VALUES ('drive2.cue', '${PE_GAMES}/${NEW_ID}/drive2.cue', ${NEW_NODE_ID});"
  sqlite3 "${DB_BLEEM}" "INSERT INTO GameManagerFiles (Name, Path, NodeId)
    VALUES ('drive2.png', '${PE_GAMES}/${NEW_ID}/drive2.png', ${NEW_NODE_ID});"
  sqlite3 "${DB_BLEEM}" "INSERT INTO GameManagerFiles (Name, Path, NodeId)
    VALUES ('Game.ini', '${PE_GAMES}/${NEW_ID}/Game.ini', ${NEW_NODE_ID});"
